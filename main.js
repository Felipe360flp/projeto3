...
class Map{
	...
	pass_local(){ 
        if(ciclo.periodo=="Noite" && map.armazena_local ===map.local[0]) 
        {
			map.armazena_local=map.local[1];
        }else if(ciclo.periodo=="Noite" && map.armazena_local===map.local[1]){
			map.armazena_local=map.local[2];
		}else if(ciclo.periodo=="Noite" && map.armazena_local==map.local[2]){
			map.armazena_regiao=map.regiao[1];
		}else{
			return;
		}
		...
		do{
			...
			if(resposta_jogador==1){
				...
				if(resposta_jogador==1){
					...
					console.log(`Saúde ${inimigo.nome}:${inimigo.saude}`);
